...
- (void)selectedAtView:(UIView *)view {
    ...
    if ([className hasPrefix:@"_DBXClickedButton_"]) {
        SEL selector = NSSelectorFromString(@"dbx_clickedAreaEdgeInsets");
        UIEdgeInsets insets = ((UIEdgeInsets (*)(id, SEL))[view methodForSelector:selector])(view, selector);
        self.buttonClickedFrameView.hidden = NO;
        CGRect expandedRect = UIEdgeInsetsInsetRect(view.frame, insets);
        self.buttonClickedFrameView.frame = [view.superview convertRect:expandedRect toView:nil];
            ...
            if (range.location != NSNotFound) {
                ...
                if (range.location != NSNotFound) {
                    ...
                    if ([name containsString:@":"]) {
                        ...
                        [info appendFormat:@"bundle:%@\n", temNames.firstObject];
                        [info appendFormat:@"imageName:%@\n", temNames.lastObject];
                    } else {
                        [info appendFormat:@"imageName:%@\n", name];
                    }
                }
